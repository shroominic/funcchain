"""Azure OpenAI chat wrapper. (from langchain_community)"""
...
class AzureChatOpenAI(ChatOpenAI):
    ...
    .. code-block:: python
        ...
        from langchain_community import AzureChatOpenAI
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        openai_api_key = values["openai_api_key"] or os.getenv("AZURE_OPENAI_API_KEY") or os.getenv("OPENAI_API_KEY")
        values["openai_api_key"] = convert_to_secret_str(openai_api_key) if openai_api_key else None
        values["openai_api_base"] = values["openai_api_base"] or os.getenv("OPENAI_API_BASE")
        values["openai_api_version"] = values["openai_api_version"] or os.getenv("OPENAI_API_VERSION")
        ...
        values["azure_ad_token"] = convert_to_secret_str(azure_ad_token) if azure_ad_token else None

        values["openai_api_type"] = get_from_dict_or_env(values, "openai_api_type", "OPENAI_API_TYPE", default="azure")
        ...
        client_params = {
            ...
            "api_key": values["openai_api_key"].get_secret_value() if values["openai_api_key"] else None,
            "azure_ad_token": values["azure_ad_token"].get_secret_value() if values["azure_ad_token"] else None,
    ...
    def _create_chat_result(self, response: Union[dict, BaseModel]) -> ChatResult:
        ...
        for res in response["choices"]:
            if res.get("finish_reason", None) == "content_filter":
                raise ValueError("Azure has not provided the response due to a content filter " "being triggered")
        ...
        if "prompt_filter_results" in response:
            chat_result.llm_output = chat_result.llm_output or {}
            chat_result.llm_output["prompt_filter_results"] = response["prompt_filter_results"]
